  ...
  async function generate() {
    ...
    try {
      const res = await fetch("/api/flashcards", {
        ...
        body: JSON.stringify({ notes: clean, count, style }),
      });
...
function FlipCard({ card }: { card: Flashcard }) {
  ...
  return (
    <button type="button" onClick={() => setFlipped((s) => !s)} className="text-left w-full group">
      <div className="rounded-2xl border border-white/10 bg-black/20 p-4 transition-transform duration-200 group-hover:scale-[1.01]">
